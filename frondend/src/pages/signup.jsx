  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(errors).length === 0) {
      try {
        ...
        console.log(res.data.status,'ddfsfdfs')
          // const response = await axios.post(`/auth/verify-email`,{email,token:res.token});
          if (res.data.status === "success") {
            toast.success('please check the email to confirm')
