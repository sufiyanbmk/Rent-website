...
function searchedProduct() {
  ...
  const { isLoading, isError, error, results, hasNextPage } = useProduct(
    pageNum,
    url,
    values,
    isFilter,
    ...
    product?.length > 0 ? (
      product.map((product, i) => {
        if (results.length === i + 1) {
          return (
            <ProductList ref={lastPostRef} key={product._id} data={product} />
          );
        }
      ...
      <div className="text-center text-gray-600 mt-6">
        No products available. Please try a different search.
      </div>
    );
    ...
    <div className="grid grid-cols-1 py-32 md:grid-cols-1 w-full">
      <section className="h-full max-h-[640px] mb-8 xl:mb-24">
        <div className="flex flex-col lg:flex-row">
          <div className="lg:ml-8 xl:ml-[135px] flex flex-col items-center lg:items-start text-center lg:text-left justify-center flex-1 px-4 lg:px-0">
            <h1
              ...
              id="top"
            >
              <span className="text-violet-700">Rent</span> You can get the
              product here...
            </h1>
        ...
        <Filter
          ...
          handleCity={handleOnchange}
          handlePriceSubmit={handlePriceSubmit}
        />
      </section>
      <div className="grid grid-cols-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-6 px-4 md:px-8 lg:px-10 xl:px-20">
        {content}
      </div>
      {isLoading && (
        <p className="text-center mt-8 md:mt-12 lg:mt-16">
          Loading More Posts...
        </p>
      )}
      <p className="text-center mt-8 md:mt-12 lg:mt-16">
